...
function FileTree({ state }: { state: BuilderState }) {
  ...
  const lines = [
    ...
    hasConvex  ? `├── <span class="text-nexa-primary">convex/</span>` : "",
    hasConvex  ? `│   ├── <span class="text-nexa-primary">schema.ts</span>        ← Nexa schema fragments` : "",
    hasConvex  ? `│   └── <span class="text-nexa-primary">nexa.ts</span>          ← function mounts → api.nexa.*` : "",
    ...
    state.pm === "pnpm" ? `├── pnpm-workspace.yaml       ← build-script approvals` : "",
